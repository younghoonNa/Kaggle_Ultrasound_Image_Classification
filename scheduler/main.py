...
def main(image_size, model_name):
    ...
    # rgb mean: [0.1776098, 0.1776098, 0.1776098]
    # rgb std: [0.037073515, 0.037073515, 0.037073515]
    ...
    # sampler = WeightedRandomSampler(weights=weights, num_samples = len(train_dataset), replacement=False)
    ...
    # sampler = WeightedRandomSamplr(weights=weights, num_samples = len(val_dataset), replacement=False)
...
if __name__ == '__main__':
    ...
    t = True
    for l, e in enumerate(reversed(timm.list_models())):
        if e == "xception71":
            exit()
        print(e)
        if e == "xcit_tiny_24_p16_384_dist":
            t = False
        else:
            if t:
                continue
